...
export default function Scan(props) {
  ...
  const { step, status, scan_id, text, pdf, sandbox } = state;

  const [poll, setPoll] = useState(0);
  const [action, setAction] = useState("pxq_pgck_get_scan_result");
  console.log("misc", step, status, scan_id, state.export, pdf, poll, action);
  useEffect(() => {
    let pollXHR;
    if (poll) {
      pollXHR = doAjax({
  ...
  return (
    <div>
      <Container>
        ...
        {1 === status ? (
          <div>
            ...
            <p>{state.export ? "Generating PDF" : "It may take some time"}</p>
          </div>
        ) : null}
        {3 === status ? (
          <p>
        ...
        {2 === status ? (
          <p>
            <strong>PDF is ready for download</strong>
          </p>
        ) : null}
        <Table>
          <tbody>
      ...
      <Button
        onClick={() => {
          window.open(window.pxq_pgck_report_url, "_blank");
        }}
